...
beforeAll(async () => {
  ...
  const [seededRegularUser] = await db.insert(users).values({ ...regularUserCredentials, username: 'regular_e2e', hashedPassword, role: 'user' }).returning();
  const [seededAdminUser] = await db.insert(users).values({ ...adminUserCredentials, username: 'admin_e2e', hashedPassword, role: 'admin' }).returning();

  regularUserId = seededRegularUser.id;
  adminUserId = seededAdminUser.id;

  const [manga] = await db.insert(mangas).values({ malId: 9999, title: "E2E Test Manga", publishedAt: new Date() }).returning();
  testMangaId = manga.id;

  const loginResUser = await request(app).post('/v1/api/auth/login').send(regularUserCredentials);
  regularUserToken = loginResUser.body.data.accessToken;

  const loginResAdmin = await request(app).post('/v1/api/auth/login').send(adminUserCredentials);
...
describe("Authentication API (/v1/api/auth)", () => {
  it("FAIL(1): should return 401 for incorrect login credentials", async () => {
    const response = await request(app).post('/v1/api/auth/login').send({ email: regularUserCredentials.email, password: 'wrongpassword' });
    expect(response.status).toBe(401);
  });

  it("SUCCESS(2): should return tokens for correct login credentials", async () => {
    const response = await request(app).post('/v1/api/auth/login').send(regularUserCredentials);
  ...
  it("SUCCESS(3): should return a new access token on refresh", async () => {
    const loginRes = await request(app).post('/v1/api/auth/login').send(regularUserCredentials);
    const refreshToken = loginRes.body.data.refreshToken;
    const response = await request(app).post('/v1/api/auth/refresh').send({ refreshToken });
    expect(response.status).toBe(200);
    expect(response.body.data).toHaveProperty('accessToken');
  });

  it("FAIL(4): should return 401 on refresh with invalid token", async () => {
    const response = await request(app).post('/v1/api/auth/refresh').send({ refreshToken: 'invalidtoken' });
    expect(response.status).toBe(401);
  });
});
...
describe("User API (/v1/api/users)", () => {
  it("SUCCESS(5): should get current user profile with /users/me", async () => {
    const response = await request(app).get('/v1/api/users/me').set('Authorization', `Bearer ${regularUserToken}`);
    expect(response.status).toBe(200);
  ...
  it("SUCCESS(7): should update user profile with /users/me", async () => {
    const response = await request(app).put('/v1/api/users/me').set('Authorization', `Bearer ${regularUserToken}`).send({ username: 'new_username' });
    expect(response.status).toBe(200);
    expect(response.body.data.username).toBe('new_username');
  });

  it("FAIL(8): should not get all users as a regular user", async () => {
    const response = await request(app).get('/v1/api/users').set('Authorization', `Bearer ${regularUserToken}`);
    expect(response.status).toBe(403);
  });

  it("SUCCESS(9): should get all users as an admin", async () => {
    const response = await request(app).get('/v1/api/users').set('Authorization', `Bearer ${adminUserToken}`);
    expect(response.status).toBe(200);
    expect(response.body.data).toBeInstanceOf(Array);
  });

  it("SUCCESS(10): admin should be able to change a user's role", async () => {
    const response = await request(app).patch(`/v1/api/users/${regularUserId}/role`).set('Authorization', `Bearer ${adminUserToken}`).send({ role: 'admin' });
    expect(response.status).toBe(200);
    expect(response.body.data.role).toBe('admin');

    await request(app).patch(`/v1/api/users/${regularUserId}/role`).set('Authorization', `Bearer ${adminUserToken}`).send({ role: 'user' });
  });
});
...
describe("Review & Comment Flow", () => {
  ...
  it("FAIL(20): should return 404 when trying to get a deleted review", async () => {
    const response = await request(app).get(`/v1/api/reviews/${createdReviewId}`);
    expect(response.status).toBe(404);
  });
});
